    ...
    google.maps.event.addListener(marker, 'click', function() {
        if (confirm("You selected " + marker.title + " . Click OK to navigate to it, or Cancel to return to the map")) {
            if (altStart) {
                ...
                console.log('Alternate start point selected.');
            }
            else {
                window.open("https://www.google.com/maps/dir/?api=1&travelmode=driving&dir_action=navigate&destination=" + marker.position.lat().toString() + "," + marker.position.lng().toString());
                 console.log('Geolocated start point selected.');
            }


        }
        else  console.log('Marker selection cancelled.');
        


    });

    //Set blink interval for marker 0 start marker
    if (index === 0) {
...
function addMapClickListener() {
    google.maps.event.addListener(map, 'click', function(event) {
        altStart = true;
        //Get the alternate start position from the map click        
        pos = event.latLng;
